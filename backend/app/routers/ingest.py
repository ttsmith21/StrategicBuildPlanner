...
async def ingest_documents(
    project_name: str = Form(..., description="Project name for this Strategic Build Plan"),
    files: List[UploadFile] = File(..., description="Documents to ingest (PDF, DOCX, TXT)")
):
    """
    ...
    try:
        ...
        for upload_file in files:
            try:
                ...
                if not is_valid:
                    logger.warning(f"File validation failed: {upload_file.filename} - {error_msg}")
                ...
                processed = await doc_processor.process_file(file_obj, upload_file.filename)
                ...
                successful += 1
                logger.info(f"Successfully processed: {upload_file.filename} ({file_size} bytes)")
