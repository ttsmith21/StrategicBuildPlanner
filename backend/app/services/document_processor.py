...
class DocumentProcessor:
    ...
    def validate_file(filename: str, file_size: int) -> Tuple[bool, str]:
        ...
        if ext not in DocumentProcessor.SUPPORTED_EXTENSIONS:
            return (
                False,
                f"Unsupported file type: {ext}. Supported: {', '.join(DocumentProcessor.SUPPORTED_EXTENSIONS)}",
            )
        ...
        if file_size > max_size_bytes:
            return (
                False,
                f"File too large: {file_size / 1024 / 1024:.1f}MB. Max: {DocumentProcessor.MAX_FILE_SIZE_MB}MB",
            )
    ...
    async def extract_text_from_pdf(file: BinaryIO, filename: str) -> str:
        ...
        try:
            ...
            extracted_text = "\n\n".join(text_parts)
            logger.info(
                f"Extracted {len(extracted_text)} characters from PDF: {filename} ({len(pdf_reader.pages)} pages)"
            )
    ...
    async def extract_text_from_docx(file: BinaryIO, filename: str) -> str:
        ...
        try:
            ...
            extracted_text = "\n\n".join(text_parts)
            logger.info(
                f"Extracted {len(extracted_text)} characters from DOCX: {filename}"
            )
