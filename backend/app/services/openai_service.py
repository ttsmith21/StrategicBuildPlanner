...
class OpenAIService:
    ...
    async def create_vector_store(
        self, name: str, file_ids: Optional[List[str]] = None
    ) -> VectorStore:
        """
        ...
        try:
            ...
            if file_ids:
                ...
                if waited >= max_wait:
                    logger.warning(
                        f"Vector store indexing timed out after {max_wait}s. "
                        "Files may not be fully searchable."
                    )
    ...
    async def upload_file(
        self, file: BinaryIO, filename: str, purpose: str = "assistants"
    ) -> str:
        """
        ...
        try:
            uploaded_file = self.client.files.create(
                file=(filename, file), purpose=purpose
            )
